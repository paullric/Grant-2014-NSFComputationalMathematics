...
%Many parameterizations of sub-grid-scale processes, such as precipitation or turbulence, perform poorly at high-resolutions since tuning factors are often nonlinearly dependent on grid resolution and tuning of these factors is typically very expensive at high resolutions.  A \textbf{global multi-resolution test bed} is needed to verify that parameterizations are capable of producing realistic results across scales.
\end{enumerate}
\vspace{-0.4cm}
...
%Atmospheric models traditionally consist of two major components.  The first component, known as the dynamical core, is responsible for solving the equations of motion which describe the temperature, pressure, wind velocity and tracer mass.  The second component, referred to as the physical parameterizations or ``physics'', handles sub-grid-scale physical processes such as radiation, moisture, chemistry and convection.  Except in experimental model codes, physical parameterizations are limited to only work on a single vertical column, meaning that the dynamical core is the entity responsible for all horizontal transport and consequently the evolution and transport of large-scale weather systems.
...
%This proposal aims to generalize and extend the work of \cite{JTTJW2005JCP} to understand how acoustic, inertia-gravity waves and the Rossby wave modes respond to different staggerings of prognostic variables, locations of interior nodes and order-of-accuracy in the context of HFEM.  
